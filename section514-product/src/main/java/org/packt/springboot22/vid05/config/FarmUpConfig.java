...
import org.packt.springboot22.vid05.security.FarmUpAuthRequestFilter;
...
public class FarmUpConfig extends WebSecurityConfigurerAdapter {  
	...
	private FarmAppAuthEntryPoint jwtAuthenticationEntryPoint;
  	...
  	public void configure(HttpSecurity http) throws Exception {  
	      http  
	         .authorizeRequests()
	         .requestMatchers(EndpointRequest.to(InfoEndpoint.class)).hasAnyAuthority("USER READ")
	         .requestMatchers(EndpointRequest.toAnyEndpoint()).hasAnyAuthority("ADMIN ENDPOINT")
	         .antMatchers("/farm/harvest**").hasAnyAuthority("ADMIN READ", "USER READ")
		 	 .antMatchers("/authenticate", "/farm/authenticate").permitAll()
